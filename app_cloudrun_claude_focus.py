...
# -*- coding: utf-8 -*-
"""
Remote Desktop for Cloud Run - Claude Code Focus Version
Enhanced with Claude Code chat focus functionality
"""
...
<head>
    <meta charset="UTF-8">
    <title>Remote Desktop - Claude Code Edition</title>
    ...
    <style>
        ...
        .title { font-size: 16px; color: #4CAF50; }
        .controls { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
        .btn { padding: 8px 12px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 11px; }
        .btn-primary { background: #2196F3; color: white; }
        .btn-warning { background: #FF5722; color: white; animation: blink 1s infinite; }
        .btn-success { background: #4CAF50; color: white; }
        .btn-claude { background: #FF6B35; color: white; }
        .btn-debug { background: #9C27B0; color: white; }
        @keyframes blink { 0%, 50% { opacity: 1; } 51%, 100% { opacity: 0.5; } }
        .status { padding: 6px 10px; border-radius: 15px; font-size: 11px; }
        ...
        .feedback.claude { background: #FF6B35; }
...
<body>
    <div class="header">
        <div class="title">Claude Code Remote Desktop</div>
        <div class="controls">
            <div id="status" class="status disconnected">READY</div>
            <button id="remote-btn" class="btn btn-warning">OFF</button>
            <button id="refresh-btn" class="btn btn-primary">REFRESH</button>
            <button id="claude-btn" class="btn btn-claude">CLAUDE</button>
            <button id="debug-btn" class="btn btn-debug">DEBUG</button>
        </div>
    </div>
    
    <div id="claude-panel" class="claude-panel">
        <h4>Claude Code Window Selection</h4>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; margin-bottom: 12px;">
            <button onclick="focusClaudePosition('top-left')" class="btn btn-claude" style="font-size: 10px;">↖️ Top-Left</button>
            <button onclick="focusClaudePosition('top-right')" class="btn btn-claude" style="font-size: 10px;">↗️ Top-Right</button>
            <button onclick="focusClaudePosition('bottom-left')" class="btn btn-claude" style="font-size: 10px;">↙️ Bottom-Left</button>
            <button onclick="focusClaudePosition('bottom-right')" class="btn btn-claude" style="font-size: 10px;">↘️ Bottom-Right</button>
        </div>
        <div style="display: flex; gap: 4px; flex-wrap: wrap;">
            <button onclick="focusClaudePosition('center')" class="btn btn-claude">🎯 Center</button>
            <button onclick="focusActiveWindow()" class="btn btn-claude">🔍 Active Window</button>
        </div>
        <hr style="margin: 12px 0; border: 1px solid #444;">
        <h5>Standard Controls</h5>
        <button onclick="scrollClaudeToBottom()" class="btn btn-claude">Scroll to Bottom</button>
        <button onclick="clearClaudeChat()" class="btn btn-claude">Clear Chat (Ctrl+L)</button>
        <div class="claude-tip">
            <strong>Multi-Window Support:</strong><br>
            • Select Claude Code window by screen position<br>
            • Active Window: Focus current active Claude<br>
            • Center: Click center of screen<br>
            • Position buttons target screen quadrants
        </div>
    </div>
    ...
    <div class="desktop-container">
        <div id="setup-instructions" class="setup-box">
            <h3>Claude Code Remote Desktop</h3>
            <div class="step">
                <strong>Connection Status:</strong><br>
                <span id="local-status">Waiting for PC client...</span>
            </div>
            <div class="step">
                <strong>Features:</strong><br>
                Screen sharing + Claude Code focus controls
            </div>
        </div>
    ...
    <div class="toolbar">
        <input type="text" id="textbox" placeholder="Type text to send to Claude Code">
    ...
    <script>
        ...
        const claudeBtnEl = document.getElementById('claude-btn');
        ...
        function updateDebugPanel() {
            if (debugMode) {
                fetch('/debug')
                    ...
                    .catch(err => {
                        debugStatusEl.innerHTML = `<span style="color: red;">Debug error: ${err}</span>`;
                    });
            }
        }
        
        // Panel toggles
        claudeBtnEl.addEventListener('click', () => {
            claudeMode = !claudeMode;
            if (claudeMode) {
                claudePanelEl.classList.add('show');
                claudeBtnEl.textContent = 'HIDE';
                showMessage('Claude Code controls enabled', 'claude');
            } else {
                claudePanelEl.classList.remove('show');
                claudeBtnEl.textContent = 'CLAUDE';
            }
        });
        ...
        function focusClaudePosition(position) {
            if (!localClientConnected) {
                showMessage('PC client not connected!', 'error');
                return;
            }
            showMessage(`Focusing ${position} Claude Code...`, 'claude');
            socket.emit('send_command', {
                command: 'claude_focus_position',
        ...
        function focusActiveWindow() {
            ...
            showMessage('Focusing active Claude Code window...', 'claude');
            socket.emit('send_command', {
                command: 'claude_focus_active'
            });
        }
        
        function scrollClaudeToBottom() {
            if (!localClientConnected) {
                showMessage('PC client not connected!', 'error');
                return;
            }
            showMessage('Scrolling to bottom...', 'claude');
            socket.emit('send_command', {
                command: 'key',
                data: { key: 'ctrl+end' }
            });
        ...
        function clearClaudeChat() {
            ...
            showMessage('Clearing Claude Code chat...', 'claude');
            socket.emit('send_command', {
                command: 'key',
                data: { key: 'ctrl+l' }
            });
        ...
        remoteBtnEl.addEventListener('click', () => {
            ...
            if (remoteMode) {
                ...
                remoteBtnEl.textContent = 'OFF';
                remoteBtnEl.className = 'btn btn-warning';
                showMessage('REMOTE MODE OFF', 'error');
            }
        });
        ...
        function handleScreenClick(event) {
            if (!clickMode || !remoteMode || !localClientConnected) {
                if (!remoteMode) showMessage('Turn ON Remote Mode first!', 'error');
        ...
        function sendTextToRemote() {
            ...
            if (!text) {
                showMessage('Enter text first!', 'error');
                return;
            }
            if (!remoteMode) {
                showMessage('Turn ON Remote Mode first!', 'error');
                return;
            }
            if (!localClientConnected) {
                showMessage('PC client not connected!', 'error');
            ...
            showMessage(`Focusing and sending: "${text.substring(0, 15)}..."`, 'claude');
            console.log('[DEBUG] Step 1: Focus bottom-right Claude first');
            
            // Step 1: Focus bottom-right Claude Code first
            socket.emit('send_command', {
                command: 'claude_focus_position',
                data: { position: 'bottom-right' }
            });
            
            // Step 2: Wait then send text
            setTimeout(() => {
                console.log('[DEBUG] Step 2: Sending text after focus');
                socket.emit('send_command', {
                    command: 'type',
                    data: { text: text }
                });
                
                // Step 3: Send Enter after text (optional)
                setTimeout(() => {
                    console.log('[DEBUG] Step 3: Sending Enter key');
                    socket.emit('send_command', {
                        command: 'key',
                        data: { key: 'enter' }
                    });
                }, 500); // Wait 500ms after text
            }, 1000); // Wait 1000ms for focus to complete
            
            textboxEl.value = '';
            console.log('[DEBUG] Text will be sent after focus');
        }
        ...
        function sendKey(keyName) {
            ...
            showMessage(`Key: ${keyName}`, 'success');
            socket.emit('send_command', {
                command: 'key',
                data: { key: keyName }
            });
        ...
        showMessage('Claude Code Edition loaded!', 'claude');
...
def handle_screenshot_data(data):
    ...
    add_debug_log(f"Screenshot received from local client {request.sid}")
...
if __name__ == '__main__':
    ...
    add_debug_log(f"Starting Cloud Run Remote Desktop (Claude Focus) on port {port}")
    try:
