...
class GitHubIssue1CommentMonitor:
    ...
    def load_github_token(self) -> str:
        ...
        # Use same token as existing system
    ...
    def get_issue_comments(self) -> Tuple[Optional[List[Dict]], Optional[str]]:
        """Fetch issue comments with ETag optimization"""
        url = f"{self.base_url}/issues/{self.issue_number}/comments"
        
        # Add ETag header for conditional requests
        headers = self.headers.copy()
        if self.state.get("comments_etag"):
            headers["If-None-Match"] = self.state["comments_etag"]
